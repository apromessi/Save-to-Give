...
class User(db.Model):
    """
    Basic user information, including username and password
    Age and zipcode included as placeholders for now - in case of future
    ...
    def accepted_challenge_info(self, user_id):
        ...
        Takes user_id as parameter and returns current and completed challenges
        as 2 lists within a list.
        """
        ...
        for ac_object in users_ac_objects:
            ...
            if ac_object.completed_at is None:
                challenge = (qty, alternative_items, original_items,
                             donation_item, donation_price, progress_percent,
                             ac_object.ac_id)
                users_current_challenges.append(challenge)
            else:
                challenge = (qty, alternative_items, original_items,
                             donation_item, donation_price,
                             ac_object.completed_at, ac_object.ac_id)
...
class Donation(db.Model):
    ...
    Connect to challenges table via association table and matching
    donation_amount and savings from challenge
    """
    ...
    def __repr__(self):
        return "<Donation Obj: %s donation_item = %s, donation_price = %s>" % (
            self.donation_id, self.donation_item, self.donation_price)


class Challenge(db.Model):
    """
    Challenges for the user connects to matching donations via savings and
    donation_amount
    """
...
class Accepted_Challenge(db.Model):
    ...
    challenge = db.relationship("Challenge", backref=db.backref(
        "accepted_challenges"))
    user = db.relationship("User", backref=db.backref(
        "accepted_challenges"))
    donation = db.relationship("Donation", backref=db.backref(
        "accepted_challenge"), uselist=False)
    ...
    def __repr__(self):
        return "<Accepted_Challenge Obj: %s user_id=%s, challenge_id=%s>" % (
...
class Progress_Update(db.Model):
    ...
    Information about progress updates can be combined to determine when a user
    has completed a challenge and can donate the money they saved.
    Progress update timestamps and amounts are displayed on charts.
    """
    ...
    accepted_challenge = db.relationship(
        "Accepted_Challenge", backref=db.backref("progress_updates"))

    def __repr__(self):
        return "<Update Obj: %s ac_id=%s, update_amt=%s, updated_at=%s" % (
...
#     # add more info for whether a particular transaction counts towards
#     # a challenge - would also allow user to input relevant transactions -
#     # or apply an existing transaction to progress?
#     user = db.relationship("User", backref=db.backref("transactions"))

#     def __repr__(self):
#         return "<Trans Obj: %s user_id=%s, category = %s, amount = %s>" % (
...
def connect_to_db(app):
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///savetogive.db'
    db.app = app
    db.init_app(app)
